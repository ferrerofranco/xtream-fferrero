...
class ModelTrainer:
    ...
    def train(self, target_year: int, extraction_date: str, model_name: str) -> None:
        ...
        self.logger.info(f"[{self.__class__.__name__}] Saving model outputs")
        self.data_controller.save_metrics(
            mape, r2, self.model_name, self.extraction_date
        )
        self.data_controller.save_forecast(
            forecast_dataframe=self.forecast.pd_dataframe(),
            model_number=model_number,
            extraction_date=extraction_date,
        )
        self.data_controller.save_alarms(alarms)
        self.data_controller.save_model(
            model=self.model,
            file_name="_".join([str(model_number), self.model_name]),
    ...
    def _get_files(self) -> None:
        (
            ...
            self.y_train_scaled,
            self.y_val,
        ) = self.data_controller.get_files_for_training(
            self.target_year, self.extraction_date
        )

    def _fit(self):
        self.logger.info(f"[{self.__class__.__name__}] Fitting model")
        self.model = TFTModel(
            hidden_size=12,
            lstm_layers=3,
    ...
    def _predict(self) -> None:
        self.logger.info(f"[{self.__class__.__name__}] Forecasting with model")
    ...
    def _evaluate_forecast(self) -> tuple[float, float]:
        ...
        self.logger.info(f"[{self.__class__.__name__}] Evaluating forecast")
    ...
    def _generate_alarms(self, model_number):
        ...
        if high_values > 0:
            ...
            alarm_contents.append(
                f"There are {high_values} values over {high_value_limit:,}"
            )
        ...
        if low_values > 0:
            ...
            alarm_contents.append(
                f"There are {low_values} values under {low_value_limit:,}"
            )
