...
class DatabaseDAO:
    ...
    def _initialize_mock_db(self) -> None:
        self.logger.info(f"[{self.__class__.__name__}] Initializing database")
    ...
    def _execute_statement(self, statement: str) -> list:
        ...
        try:
            with closing(sqlite3.connect(self.db_path)) as con, con, closing(
                con.cursor()
            ) as cur:
            ...
            self.logger.info(f"[{self.__class__.__name__}] {statement}")
            raise

    def save_metrics(
        self, mape: float, r2: float, model_name: str, extraction_date: str
    ) -> None:
        self.logger.info(f"[{self.__class__.__name__}] Saving metrics to DB")
