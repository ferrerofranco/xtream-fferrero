class ModelTrainer:
    ...
    def compare_month(self, month_nbr, val_ts):
        ...
        val_ts.pd_dataframe().query("Date.dt.month==@month_nbr").Load.plot.line(
            legend=True
        )
        self.baseline_forecast.pd_dataframe().query(
            "Date.dt.month==@month_nbr"
        ).Load.plot.line(legend=True)
        self.improved_forecast.pd_dataframe().query(
            "Date.dt.month==@month_nbr"
        ).Load.plot.line(legend=True)
