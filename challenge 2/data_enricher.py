...
class DataEnricher:
    ...
    def _get_y(self):
        import pandas as pd
        y_ts = TimeSeries.from_dataframe(df=self.base_dataframe,time_col='Date',value_cols='Load', freq='D',) 
    ...
    def _decompose_base_dataframe(self):
        ...
        mstl_res = MSTL(self.base_dataframe.set_index('Date')[:'2018-12-31'], periods=(7,365)).fit()
        ...
        self.complete_trend_df = pd.concat([trend_df,last_trend_year],ignore_index=True)
        self.complete_seasonal_df = pd.concat([seasonal_df,last_seasonal_year],ignore_index=True)

    def _set_base_dataframe(self, power_load_df):
        self.base_dataframe = power_load_df.reset_index().query("Date<='2019-12-31'").copy()

    def _get_weekend_ts(self):
        base_copy = self.base_dataframe.copy()
        base_copy['is_weekend'] = base_copy.Date.apply(lambda x: 1 if x.isoweekday()>5 else 0)
    ...
    def _get_covariates_ts(self):
        ...
        trend_ts = TimeSeries.from_dataframe(df=self.complete_trend_df,time_col='Date',value_cols='trend', freq='D',) 
        # seasonal_week_ts = TimeSeries.from_dataframe(df=self.complete_seasonal_df,time_col='Date',value_cols='seasonal_week', freq='D',) 
        seasonal_year_ts = TimeSeries.from_dataframe(df=self.complete_seasonal_df,time_col='Date',value_cols='seasonal_year', freq='D',) 
